...
function exportCSV(places: Place[]) {
  const headers = [
    ...
    'Trails (10mi)',
    'Summer 1980s (°F)', 'Summer Recent (°F)', 'Summer Warming (°F/dec)',
    'Winter 1980s (°F)', 'Winter Recent (°F)', 'Winter Warming (°F/dec)',
  ...
  const rows = places.map((p, i) => [
    ...
    p.prismJanTminF ?? '',
    p.trailMiles10mi ?? '',
    p.summerF1980s ?? '', p.summerFRecent ?? '', p.summerTrendFDec ?? '',
    p.winterF1980s ?? '', p.winterFRecent ?? '', p.winterTrendFDec ?? '',
